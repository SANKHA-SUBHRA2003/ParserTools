function formatLRTable(lrTable) {
	var result = "<h3 align='center'><b>With the information above, we generate the LR Parsing Table for this grammar.:</b></div><table border=\"1\" class='table table-bordered'>";
	var grammar = lrTable.grammar;
	result += "<thead>";
	result += "<tr><th colspan=\"" + (1 + grammar.terminals.length + 1 + grammar.nonterminals.length) + "\">LR Parsing Table</th></tr><tr><th rowspan=\"2\"> States </th><th rowspan=\"1\" colspan=\"" + (grammar.terminals.length + 1) + "\"> Action </th><th colspan=\"" + grammar.nonterminals.length + "\"> Goto </th></tr>\n<tr>";
